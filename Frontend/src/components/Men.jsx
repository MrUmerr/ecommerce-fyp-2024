  ...
  useEffect(() => {
    const getShop = async () => {
      try {
        ...
        const filteredData = res.data.filter((data) => data.Category === "Men");
        console.log(filteredData); // Logs only the "test" category data
  ...
  return (
    <>
      <div className='max-w-screen-2xl container mx-auto md:px-16 px-2'>
        <div className='mt-28'>
          <h1 className='text-3xl items-center justify-center font-bold'>Men's</h1>
        </div>
        ...
        <div className='mt-4 pt-2 grid grid-cols-2 md:grid-cols-5'>
          {shop.length > 0 ? (
            ...
            <p>No items found in the "test" category.</p> // Fallback message
          )}
        </div>
      </div>
    </>
